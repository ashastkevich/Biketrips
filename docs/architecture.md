...
Проект не использует дополнительный абстрактный repository-слой поверх TypeORM на старте. Если в будущем появится реальная необходимость менять ORM или изолировать сложную persistence-логику, такой слой можно будет добавить точечно для отдельных модулей.
